            ...
            <ol class="flex items-center space-x-4">
                ...
                <li>
                    <div class="flex items-center">
                        ...
                        <span class="ml-4 text-sm font-medium text-blue-600">Tambah Baru</span>
                    </div>
                </li>
            </ol>
        </nav>
        ...
        <div class="bg-white rounded-2xl shadow-xl overflow-hidden">
            <div class="relative bg-gradient-to-r from-blue-600 to-indigo-700 px-8 py-6">
                ...
                <div class="relative z-10 flex items-center">
                    ...
                    <h2 class="text-xl font-bold text-white">Form Pengumuman Baru</h2>
                </div>
            </div>

            <form action="{{ route('guru.announcements.store') }}" method="POST" class="p-8">
                @csrf
                ...
                <div class="mb-8">
                    ...
                    <div class="mt-1 relative rounded-md shadow-sm">
                        <input type="text" name="title" id="title" class="block w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition duration-200 @error('title') border-red-500 @enderror" value="{{ old('title') }}" placeholder="Contoh: Libur Semester Genap">
                        @error('title')
                ...
                <div class="mb-8">
                    ...
                    <textarea name="description" id="description" rows="8" class="block w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition duration-200 @error('description') border-red-500 @enderror" placeholder="Tulis detail pengumuman di sini...">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @else
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
                    ...
                    <div>
                        ...
                        <div class="mt-1 relative">
                            <select name="urgency" id="urgency" class="block w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition duration-200 @error('urgency') border-red-500 @enderror">
                                <option value="low" {{ old('urgency') == 'low' ? 'selected' : '' }}>Rendah (Informasi biasa)</option>
                                <option value="medium" {{ old('urgency') == 'medium' ? 'selected' : '' }}>Sedang (Penting)</option>
                                <option value="high" {{ old('urgency') == 'high' ? 'selected' : '' }}>Tinggi (Sangat penting)</option>
                            </select>
                    ...
                    <div>
                        ...
                        <div class="mt-1 relative rounded-md shadow-sm">
                            <input type="date" name="expires_at" id="expires_at" class="block w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition duration-200 @error('expires_at') border-red-500 @enderror" value="{{ old('expires_at') }}">
                            @error('expires_at')
                                <div class="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
                                    <svg class="h-5 w-5 text-red-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path>
...
@section('scripts')
<script>
    // Optional: Add any JavaScript for form enhancements (e.g., dynamic validation)
